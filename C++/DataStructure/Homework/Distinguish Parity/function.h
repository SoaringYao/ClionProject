...
void separateOddAndEven(SqList &L) {
    int leftPointer = 0;
    int rightPointer = L.length - 1;

    while (leftPointer < rightPointer) {
        while (L.elem[leftPointer] % 2 == 1) {
            leftPointer++;
        }
        while (L.elem[rightPointer] % 2 == 0) {
            rightPointer--;
        }

        if (leftPointer < rightPointer) {
            // 交换左右指针指向的元素
            swap(L.elem[leftPointer], L.elem[rightPointer]);
        }
    }
}
