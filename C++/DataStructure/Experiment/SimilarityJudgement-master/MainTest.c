...
int main() {
  ...
  FILE *fp;               // 声明文件指针
  fp = fopen("../FileNeeded/keywords.txt",
             "r");  // 打开存储C语言关键字的文件并且规定为只读
  ...
  */
  ...
  FILE *fp1;
  int ch1;
  if ((fp1 = fopen("../TestFile/chengxu1.txt", "r")) == NULL) {
    printf("打开测试文件失败");
    exit(1);
  }
  ...
  while ((ch1 = getc(fp1)) != EOF) {
    if ((ch1 >= 'a') && (ch1 <= 'z')) {
      ...
      if (search_str(root, words1)) hash_table_lookup(words1);
      i = 0;
      for (int j = 0; j < 10; j++) {
        words1[j] = '\0';
      }
...
void chengxu() {
  ...
  */
  ...
  // printf("哈希表已建立完成！共有%d个结点\n",
  // hash_table_size);//打印提示字符串 printf("输出哈希表：\n");
  // hash_table_print();                                       //输出哈希表
  /*
  printf("请输入要查询的字符串:\n");                        //检验查询
  scanf("%s", str);
  ...
  */
  ...
  FILE *fp1;
  int ch1;
  if ((fp1 = fopen("../TestFile/chengxu2.txt", "r")) == NULL) {
    printf("打开测试文件失败");
    exit(1);
  }
